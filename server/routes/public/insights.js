...
<main id="main"><section class="hero-i"><img class="blue-band-logo" src="/assets/kdh-law-logo-transparent.png" alt="" aria-hidden="true"><div class="hero-inner"><div><p class="eyebrow">KDH Insights</p><h1>Legal analysis for decisions that matter.</h1></div><p>Practical perspectives on Kenyan commercial law, disputes, investment, technology, real estate and the legal issues shaping business across Africa.</p></div></section><section class="main-i"><div class="toolbar"><div class="count">${posts.length} ${posts.length === 1 ? 'publication' : 'publications'}</div><div class="toggle" role="group" aria-label="Choose Insights layout"><button id="grid-view" aria-pressed="true" type="button">Grid</button><button id="list-view" aria-pressed="false" type="button">List</button></div></div><div class="grid" id="grid" data-count="${posts.length}">${cards}</div></section></main>
